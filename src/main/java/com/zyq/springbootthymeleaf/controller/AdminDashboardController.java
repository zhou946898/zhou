...
public class AdminDashboardController {
    ...
    @Autowired
    private PortfolioProjectRepository portfolioProjectRepository; // 注入作品集仓库
    ...
    public String portfolioDashboard(
            @RequestParam(value = "title", required = false) String title,
            Model model) {
        List<PortfolioProject> projects = title != null
                ? portfolioProjectRepository.findByTitleContaining(title)
                : portfolioProjectRepository.findAll();
        model.addAttribute("projects", projects);
        return "fragments/portfolio_table";
    }
}